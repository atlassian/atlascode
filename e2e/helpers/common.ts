...
/**
 * Helper function to get the Pull Request iframe
 */
export const getPRFrame = async (page: Page): Promise<Frame> => {
    const webviewFrame = page.frameLocator('iframe.webview');
    // Try multiple possible iframe titles/selectors for PR
    const possibleSelectors = [
        'iframe[title="Pull Request 123"]',
        'iframe[title*="Pull Request"]',
        'iframe[title*="pull-request"]',
        'iframe[src*="pullrequest"]',
        'iframe[src*="pull-request"]',
        'iframe:last-child', // fallback to last iframe
    ];
    for (const selector of possibleSelectors) {
        try {
            const frameHandle = await webviewFrame.locator(selector).elementHandle({ timeout: 2000 });
            if (frameHandle) {
                const prFrame = await frameHandle.contentFrame();
                if (prFrame) {
                    return prFrame;
                }
            }
    ...
    throw new Error(`No suitable PR iframe found. Available iframe titles: ${iframeTitles.join(', ')}`);
};
