...
- **Jira/Bitbucket (Server/Data Center)**: Fixed a credential collision when connecting to more than one Server/Data Center site with Personal Access Token (PAT) auth using the same username. The stored credential ID for PAT logins was scoped only by product (`jira`/`bitbucket`) instead of by host, so authenticating to a second site with the same username silently overwrote the first site's stored token, causing the first site to disconnect with an auth error. The credential ID is now always scoped to the site's host.
