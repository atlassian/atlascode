...
describe('RovoDevChatProvider', () => {
    ...
    describe('ui_changes_detected SSE event', () => {
        ...
        it('should post ShowLivePreviewButton message when ui_changes_detected event is received', async () => {
            const mockReadableStream = new ReadableStream({
                start(controller) {
                    controller.enqueue(new TextEncoder().encode('event: ui_changes_detected\ndata: {}\n\n'));
    ...
    describe('usage SSE event', () => {
        beforeEach(async () => {
            await chatProvider.setReady(mockApiClient);
        });

        it('should replace {title} placeholder in rate limit exceeded message', async () => {
            const mockPrompt: RovoDevPrompt = {
                text: '/usage',
                context: [],
            };
            ...
            const usageData = {
                ...
                data: {
                    content: {
                        ...
                        exceeded_message: {
                            message: "You've exceeded your {title} limit. {ctaLink}",
            ...
            (rovoDevUtils.usageJsonResponseToMarkdown as jest.Mock).mockReturnValue({
                usage_response: 'usage markdown',
                alert_message: usageData.data.content.exceeded_message,
            });

            const mockReadableStream = new ReadableStream({
                start(controller) {
                    controller.enqueue(
                        new TextEncoder().encode(`event: usage\ndata: ${JSON.stringify(usageData)}\n\n`),
                    );
                    controller.enqueue(new TextEncoder().encode('event: close\ndata: \n\n'));
                    controller.close();
                },
            ...
            // Should show a warning dialog with {title} replaced by the actual title value
            expect(mockWebview.postMessage).toHaveBeenCalledWith(
                expect.objectContaining({
                    type: RovoDevProviderMessageType.ShowDialog,
                    message: expect.objectContaining({
                        ...
                        text: "You've exceeded your Monthly Credits limit. ",
                        event_kind: '_RovoDevDialog',
                        ctaLink: { link: 'https://example.com', text: 'Upgrade' },
                    }),
                }),
