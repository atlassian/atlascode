...
describe('RovoDevChatProvider', () => {
    ...
    describe('deferred_request SSE event with args as pre-parsed object', () => {
        // Regression test: "[object Object] is not valid json" in Boysenberry chat.
        // The SSE parser does a top-level JSON.parse on event data, so nested fields like
        // `args` may arrive as already-parsed objects instead of JSON strings. Previously,
        // calling JSON.parse(args) on a non-string would coerce it to "[object Object]" and throw.

        beforeEach(async () => {
            await chatProvider.setReady(mockApiClient);
        });

        it('should handle ask_user_questions with args as a pre-parsed object (not a JSON string)', async () => {
            const argsObject = {
                questions: [{ question: 'Which framework?', options: [{ label: 'React' }, { label: 'Vue' }] }],
            };
            const deferredRequestData = {
                calls: [{ tool_name: 'ask_user_questions', tool_call_id: 'call-123', args: argsObject }],
            };
            ...
            const mockReadableStream = new ReadableStream({
                start(controller) {
                    controller.enqueue(
                        new TextEncoder().encode(
                            `event: deferred-request\ndata: ${JSON.stringify(deferredRequestData)}\n\n`,
                        ),
                    );
                    controller.enqueue(new TextEncoder().encode('event: close\ndata: \n\n'));
                    controller.close();
            ...
            await chatProvider.executeChat({ text: 'which framework?', context: [] }, []);

            // Should show the deferred ask_user_questions dialog — not an error
            expect(mockWebview.postMessage).toHaveBeenCalledWith(
                expect.objectContaining({
            ...
            // Must NOT display a "[object Object] is not valid json" error dialog
            const errorCalls = mockWebview.postMessage.mock.calls.filter(
                (call) => call[0].type === RovoDevProviderMessageType.ShowDialog && call[0].message?.type === 'error',
            );
            expect(errorCalls).toHaveLength(0);
        });

        it('should handle exit_plan_mode with args as a pre-parsed object', async () => {
            const argsObject = { plan: 'Step 1: do X\nStep 2: do Y' };
            const deferredRequestData = {
                calls: [{ tool_name: 'exit_plan_mode', tool_call_id: 'call-456', args: argsObject }],
            };
            ...
            await chatProvider.executeChat({ text: 'create a plan', context: [] }, []);
        ...
        it('should handle ask_user_questions with args as a JSON string (existing behaviour)', async () => {
            const argsObject = {
                ...
                calls: [
                    {
                        ...
                        args: JSON.stringify(argsObject), // original expected format: a JSON string
                    },
                ],
            };
            ...
            await chatProvider.executeChat({ text: 'pick something', context: [] }, []);
